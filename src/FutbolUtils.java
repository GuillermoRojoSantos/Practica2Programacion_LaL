...
public class FutbolUtils {
	...
	public static void main(String[] args) {
		...
		Equipo equipo = crearUnEquipo();
		String nombreArchivo = "miEquipo.txt";
		
		try {
			FileWriter fileEquipo= new FileWriter(nombreArchivo,false);
			
			BufferedWriter bufferedEquipo= new BufferedWriter(fileEquipo);
			
			bufferedEquipo.write(equipo.getNombre());
			bufferedEquipo.newLine();
			for(int i=0;i<equipo.getJugadores().length;i++) {
				Jugador jug= equipo.getJugadores()[i];
				bufferedEquipo.newLine();
				bufferedEquipo.write(jug.getNombre());
				bufferedEquipo.newLine();
				bufferedEquipo.write(jug.getApellidos());
				bufferedEquipo.newLine();
			}
			bufferedEquipo.close();
		...
		System.out.println("Proceso de escritura de Jugador.");
	...
	public static Equipo crearUnEquipo() {
		...
		String[] nombreEquipos = { "Real Madrid", "Sevilla", "Betis", "Atletico de Madrid", "Barcelona",
				"Real Sociedad", "Villarreal", "Rayo Vallecano", "Athletic de Bilbao", "Valencia C.F.", "Osasuna",
				"Celta de Vigo", "RCD Espanyol", "Granada", "Elche C.F.", "Getafe", "R.C.D Mallorca", "Cadiz", "Alaves",
				"Levante" };
	...
	private static Entrenador crearEntrenador(Equipo equipo) {
		...
		String[] apellidos = { "Messi", "Vinicius", "Cristiano", "Ronaldo", "PiquÃ©", "Bale (lesionado)", "Amunike",
				"N'kono", "ButragueÃ±o", "SanchÃ­s", "Neymar", "Batistuta", "Maradona", "PelÃ©", "Beckenbauer" };
		Entrenador entrenador = new Entrenador();
	...
	private static Jugador[] crearJugadores(int numeroJugadores, int edad, Equipo equipo) {
		...
		String[] nombres = { "Antonio", "Pepito", "Alejandra", "Ismael", "Hugo", "Oliver", "Kalesi", "Ingrid", "Astrid",
				"Indira", "Jenny", "Jessi", "Vane", "Joel", "Bruno", "Sasha", "Billie", "Masha", "Pingu" };
		String[] apellidos = { "Messi", "Vinicius", "Cristiano", "Ronaldo", "Pique", "Bale (lesionado)", "Amunike",
				"N'kono", "Butragueño", "Sanchis", "Neymar", "Batistuta", "Maradona", "Pele", "Beckenbauer" };
